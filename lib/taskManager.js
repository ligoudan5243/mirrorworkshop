...
export async function updateMasterTaskProgress(env, taskId, updates) {
    ...
    // 如果是增量更新 processedFiles，则累加
    if (updates.processedFiles !== undefined) {
        // 直接设置，因为传入的是新的总数
        task.processedFiles = updates.processedFiles;
    }
    if (updates.failedFiles !== undefined) {
        task.failedFiles = updates.failedFiles;
    }
    if (updates.completedBatches !== undefined) {
        task.completedBatches = updates.completedBatches;
    }
